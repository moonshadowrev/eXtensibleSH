...
# OS: debian (Ubuntu, Debian, etc.)
# Description: Installs and tunes NGINX based on server profile.
...
exec >> "$LOG_FILE" 2>&1
echo "NGINX plugin for Debian started at $(date)"

# Install NGINX if not present
if ! command -v nginx >/dev/null 2>&1; then
  echo "Installing NGINX..."
  ${PREFIX}apt update -y
  ${PREFIX}apt install nginx -y
fi
...
select profile in "Low end server (1 vCPU + 4GB RAM)" "Mid level (4 vCPU + 16GB RAM)" "High level (8+ vCPU + 32GB+ RAM)"; do
  case $profile in
    "Low end server (1 vCPU + 4GB RAM)")
      echo "Tuning NGINX for low-end server"
      #customizations
      ...
      echo "fs.file-max=10000" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.core.somaxconn=128" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.ipv4.tcp_max_syn_backlog=128" | ${PREFIX}tee -a /etc/sysctl.conf
      ${PREFIX}sysctl -p
      ${PREFIX}systemctl restart nginx
      break
      ;;
    "Mid level (4 vCPU + 16GB RAM)")
      echo "Tuning NGINX for mid-level server"
      ...
      echo "fs.file-max=50000" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.core.somaxconn=512" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.ipv4.tcp_max_syn_backlog=1024" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.ipv4.tcp_fin_timeout=30" | ${PREFIX}tee -a /etc/sysctl.conf
      ${PREFIX}sysctl -p
      ${PREFIX}systemctl restart nginx
      break
      ;;
    "High level (8+ vCPU + 32GB+ RAM)")
      echo "Tuning NGINX for high-level server"
      ...
      echo "fs.file-max=100000" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.core.somaxconn=1024" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.ipv4.tcp_max_syn_backlog=2048" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.ipv4.tcp_fin_timeout=15" | ${PREFIX}tee -a /etc/sysctl.conf
      echo "net.core.netdev_max_backlog=5000" | ${PREFIX}tee -a /etc/sysctl.conf
      ${PREFIX}sysctl -p
      ${PREFIX}systemctl restart nginx
      break
      ;;
...
echo "NGINX tuned and restarted."
