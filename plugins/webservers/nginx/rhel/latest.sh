...
# OS: rhel (CentOS, Fedora, RHEL, etc.)
...
exec >> "$LOG_FILE" 2>&1
echo "NGINX plugin for RHEL started at $(date)"

# Install NGINX if not present
if ! command -v nginx >/dev/null 2>&1; then
  echo "Installing NGINX..."
  ${PREFIX}yum install -y epel-release || ${PREFIX}dnf install -y epel-release
  ${PREFIX}yum install -y nginx || ${PREFIX}dnf install -y nginx
fi
...
select profile in "Low end server (1 vCPU + 4GB RAM)" "Mid level (4 vCPU + 16GB RAM)" "High level (8+ vCPU + 32GB+ RAM)"; do
  case $profile in
    "Low end server (1 vCPU + 4GB RAM)")
      ...
      ${PREFIX}sed -i 's/gzip on;/gzip off;/' /etc/nginx/nginx.conf  # Disable for low resources
      # Kernel tuning
